...
const birthdayView = (user) => {
  if (!user.birthday) return null
  return <div style={styles.birtdayText}>{ `${user.birthday === '0' ? '女' : '男'} | ${user.birthday}`}</div>
}

const buttomList = (token) => {
  const array = [
  ...
  const array2 = [
    ...
    { title: '修改密码', icon: 'password', type: 'simple-line-icon', navigateUrl: 'setPassword' },
...
export default connect(mapStateToProps, {signin, queryUser, queryPatients})(ProfileScreen)
