...
class InpatientListScreen extends Component {
  constructor (props) {
    super(props)
    this.state = {
      selectInpatientId: props.selectInpatientId,
  ...
  async autoSignin () {
    ...
    if (userId && !this.props.selectInpatientId) {
      ...
      this.queryInpatient(this.props.selectInpatientId)
    }
  }
  ...
  renderPatientList () {
    ...
    return (
      <FilterCard>
        ...
        />
        <p style={{color: '#fff', width: '30%', textAlign: 'center'}}
          onClick={() => {Router.push('/inpatient/query_list')}}>绑住院卡</p>
      </FilterCard>
    )
  }
