...
class DepartmentLevel2Screen extends Component {
  ...
  filterCard(departments) {
		...
		newdepartments = departments && departments.filter((item) => item.id === curstatus)
    ...
    return (
      <div>
          ...
          data={this.props.departments}
				...
				{
					departments.childs && departments.childs.length > 0 ?
						departments.childs.map((department, iKey) => {
...
export default connect(mapStateToProps, { queryDepartments, showPrompt, updateDepartment })(DepartmentLevel2Screen)
